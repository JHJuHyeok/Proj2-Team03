...
public class StageJsonCreator : EditorWindow
{
    ...
    private void DrawStageItem(List<StageData> stages, int index)
    {
        ...
        GUILayout.Space(5);
        stage.goldDrop = EditorGUILayout.LongField("최소 골드", stage.goldDrop);
        stage.expDrop = EditorGUILayout.IntField("경험치 획득량", stage.expDrop);
        stage.cubeCount = EditorGUILayout.IntField("강화큐브 획득량", stage.cubeCount);
        stage.cubeRate = EditorGUILayout.FloatField("강화큐브 획득확률", stage.cubeRate);
        GUILayout.Space(5);
